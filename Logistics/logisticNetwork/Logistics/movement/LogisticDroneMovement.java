...
public class LogisticDroneMovement extends MovementModel {
	...
	public LogisticDroneMovement(Settings settings) {
		...
		System.out.println("initialize Destitnation List@LogsitcDroneMovementmodel");

		this.PList = new DataPointList();
		System.out.println("initialize Data Point List@LogisticDroneMovementmodel");

		
		
		this.BList = new DestinationList();
		System.out.println("initialize Base List@LogisticDroneMovementmodel");
	...
	public Coord getInitialLocation() {
		...
		//自身の基地を初期配置にする。
		Point Base=getMyBase();
		Coord c = new Coord(Base.getX(),Base.getY());
		this.lastWaypoint = c;
		
		readListDest();
		readListDataPoint();
		System.out.println("test1 Dlist:"+DList);
		System.out.println("test2 Plist:"+PList);
		System.out.println("test3 Blist:"+BList);
	...
	public void readListDest() {
		...
		try {
			...
			destp = new Point(keep.getX(),keep.getY(),keep.getX(),keep.getY());
			DList.addList(destp);
		...
		try {
			...
			while ((data = br.readLine()) != null) {
				...
				if((data=br.readLine())==null) {
					...
					y=Double.parseDouble(data);
					int i = DList.pointsList.size();
					datap = new Point(x,y,DList.pointsList.get(i-1).getX(),DList.pointsList.get(i-1).getY());
					PList.addList(datap);
